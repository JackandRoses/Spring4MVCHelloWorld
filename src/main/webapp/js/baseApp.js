		...
		if(this.status==0){
			 $.ajax({  
	            ...
	            success:function(data,textStatus){
	                ...
	                if(option.timer<=0){
	                	longConntionObj.create(option);
	                }else{
	                	window.setTimeout(function(){longConntionObj.create(option);},option.timer*1000);
	                }
	            },  
...
baseApp.prototype.createModal = function(modalId,title){
	$("#"+modalId).remove();
	var s = "<div class='modal fade' drag='true' id='"+modalId+"' tabindex='-1' role='dialog' aria-labelledby='"+modalId+"Label' aria-hidden='true'>"+
			   "<div class='modal-dialog modal-lg'>"+
				  "<div class='modal-content'>"+
					 "<div class='modal-header'>"+
						"<button type='button' class='close' data-dismiss='modal' aria-hidden='true'>&times;</button>"+
						"<h4 class='modal-title' id='"+modalId+"Label'>"+
							"<i class='ion-compose' style='font-size:26px;'></i>"+
						   "<span>"+title+"</span>"+
						"</h4>"+
					 "</div>"+
					 "<div class='modal-body "+modalId+"-body'></div>"+
					 "<div class='modal-footer'>"+
						"<button type='button' class='btn btn-default' data-dismiss='modal'>取消</button>"+
						"<button type='button' class='btn btn-primary "+modalId+"-save'>保存</button>"+
					 "</div>"+
				  "</div>"+
			   "</div>"+
			"</div>";
